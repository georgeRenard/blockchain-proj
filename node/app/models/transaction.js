...
class Transaction {
    ...
    validateSignature() {
        this.senderPubKey = {
            x: this.senderPubKey[0],
            y: this.senderPubKey[1]
        };
    ...
    static fromJSON(tx) {
        return new Transaction(tx.from, tx.to, tx.amount,
            tx.timestamp, tx.senderPubKey,
            tx.signature, tx.hash, tx.blockIndex,
            tx.paid || tx.success);
